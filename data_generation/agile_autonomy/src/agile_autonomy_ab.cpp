#include "agile_autonomy/agile_autonomy_ab.h"
...
AgileAutonomyAB::AgileAutonomyAB(const ros::NodeHandle& nh,
  ...
  : nh_(nh), pnh_(pnh)
{
  ...
  visualizer_ = std::make_shared<visualizer::Visualizer>(nh_, pnh_, "agile_autonomy_ab");
  odometry_sub_ = nh_.subscribe("ground_truth/odometry", 1, &AgileAutonomyAB::odometryCallback, this, ros::TransportHints().tcpNoDelay());
  setup_logging_sub_ = nh_.subscribe("save_pc", 1, &AgileAutonomyAB::setupLoggingCallback, this);
  traj_sub_ = nh_.subscribe("trajectory_predicted", 1, &AgileAutonomyAB::trajectoryCallback, this);
  start_flying_sub_ = pnh_.subscribe("start_flying", 10, &AgileAutonomyAB::stopFlyingCallback, this);
  land_sub_ = nh_.subscribe("autopilot/land", 1, &AgileAutonomyAB::landCallback, this);
  off_sub_ = nh_.subscribe("autopilot/off", 1, &AgileAutonomyAB::offCallback, this);
  force_hover_sub_ = nh_.subscribe("autopilot/force_hover", 1, &AgileAutonomyAB::forceHoverCallback, this);
  completed_global_plan_sub_ = nh_.subscribe("completed_global_plan", 1, &AgileAutonomyAB::completedGlobalPlanCallback, this);
  waypoint_sub_ = nh_.subscribe("goal_point", 1, &AgileAutonomyAB::wayPointCallback, this);
  depth_sub_ = nh_.subscribe("/depth", 1, &AgileAutonomyAB::depthCallback, this);
  ...
  mission_stop_pub_ = pnh_.advertise<std_msgs::Bool>("/test_primitive/completed_planning", 10);
  ...
  pc_path_pub_ = pnh_.advertise<avoid_msgs::PointCloudInfo>("/pc_path", 1);
  save_timer_ = nh_.createTimer(ros::Duration(1.0 / save_freq_),
                                &AgileAutonomyAB::saveLoop, this);
  sample_times_.clear();
...
void AgileAutonomyAB::setupLoggingCallback(const std_msgs::BoolConstPtr& msg) {
  if (msg->data) {
    ROS_INFO(
...
void AgileAutonomyAB::depthCallback(const sensor_msgs::ImageConstPtr& msg)
{
  std::lock_guard<std::mutex> guard(img_mtx);
  cv::Mat received = cv_bridge::toCvShare(msg, "mono16")->image;
  depth = cv::Mat(received.rows, received.cols, CV_16UC1);
  received.copyTo(depth);
}

void AgileAutonomyAB::saveDepth(const std::string curr_data_dir, const int frame_counter)
{
  if (!curr_data_dir.empty()) {
    ...
    cv::imwrite(depth_img_filename, depth);
    // cv::imwrite(gt_depth_img_filename, gt_depth_uint16);}
  }
}

void AgileAutonomyAB::wayPointCallback(const nav_msgs::PathConstPtr& msg)
{
  goal.x() = msg->poses[0].pose.position.x;
  goal.y() = msg->poses[0].pose.position.y;
  goal.z() = msg->poses[0].pose.position.z;
}

void AgileAutonomyAB::stopFlyingCallback(const std_msgs::BoolConstPtr& msg) {
...
void AgileAutonomyAB::landCallback(const std_msgs::EmptyConstPtr& msg) {
...
void AgileAutonomyAB::offCallback(const std_msgs::EmptyConstPtr& msg) {
...
void AgileAutonomyAB::forceHoverCallback(const std_msgs::EmptyConstPtr& msg) {
  ROS_INFO("Received force hover command, stopping maneuver execution!");
  ROS_INFO("Switching to kOff");
  state_machine_ = StateMachine::kOff;
  std_msgs::Bool stop;
  stop.data=false;
  mission_stop_pub_.publish(stop);
}

void AgileAutonomyAB::computeManeuver(const bool only_expert) {
  ...
  quadrotor_common::TrajectoryPoint start_state;
  {
    ...
    std::cout<<"start_state.position: "<<start_state.position.transpose()<<std::endl;
  ...
  Eigen::Vector3d dire = (goal - start_state.position).normalized();
  double end_yaw = yawFromQuaternion(q_body_world);

  Eigen::Vector3d end_position_1 = start_state.position + dire * maneuver_velocity_ / 2.0;
  Eigen::Vector3d end_velocity_1 = dire * maneuver_velocity_;
  Eigen::Vector3d end_position_2 = goal - dire * maneuver_velocity_ / 2.0;
  Eigen::Vector3d end_velocity_2 = dire * maneuver_velocity_;
  Eigen::Vector3d end_position_3 = goal;
  ...
  // for(std::list<quadrotor_common::Trajectory>::iterator it1=acrobatic_sequence.getManeuverList().begin(); it1!=acrobatic_sequence.getManeuverList().end(); it1++)
  // {
  //   for(std::list<quadrotor_common::TrajectoryPoint>::iterator it2 = it1->points.begin(); it2 != it1->points.end(); it2++)
  //     std::cout<<it2->position.transpose()<<std::endl;
  // }
  acrobatic_trajectory_.points.clear();
  fuseTrajectories(acrobatic_sequence.getManeuverList(),
                   &acrobatic_trajectory_);
  // std::cout<<"acrobatic_trajectory_.points.size(): "<<acrobatic_trajectory_.points.size()<<std::endl;
  // for(std::list<quadrotor_common::TrajectoryPoint>::iterator iter = acrobatic_trajectory_.points.begin(); iter != acrobatic_trajectory_.points.end(); ++iter)
  //   std::cout<<iter->position.transpose()<<std::endl;
  ...
  if (success) {
    ...
    std::cout<<"min_corner: "<<min_corner.transpose()<<" max_corner: "<<max_corner.transpose()<<std::endl;
    // flightmare_bridge_->generatePointcloud(min_corner, max_corner,
    //                                        curr_data_dir_);
    // open log file
    avoid_msgs::PointCloudInfo pc_info;
    pc_info.header.stamp = ros::Time::now();
    pc_info.curr_dir = curr_data_dir_+"/";
    pc_info.range.resize(3);
    pc_info.origin.resize(3);
    pc_info.range[0] = max_corner[0] - min_corner[0] + 14;
    pc_info.range[1] = max_corner[1] - min_corner[1] + 12;
    pc_info.range[2] = max_corner[2] - min_corner[2] + 8;
    pc_info.origin[0] = (max_corner[0] + min_corner[0])/2.0;
    pc_info.origin[1] = (max_corner[1] + min_corner[1])/2.0;
    pc_info.origin[2] = (max_corner[2] + min_corner[2])/2.0 + 2.0;
    pc_path_pub_.publish(pc_info);
...
void AgileAutonomyAB::saveLoop(const ros::TimerEvent& time) {
  ...
  if (received_state_est_.isValid()) {
    ...
    if (state_machine_ == StateMachine::kExecuteExpert ||
      ...
      {
        std::lock_guard<std::mutex> guard(img_mtx);
        saveDepth(curr_data_dir_, frame_counter_);
      }
...
double AgileAutonomyAB::yawFromQuaternion(const Eigen::Quaterniond& q) {
...
void AgileAutonomyAB::odometryCallback(const nav_msgs::OdometryConstPtr& msg) {
  ...
  if (!acrobatic_trajectory_.points.empty()) {
    ...
    // std::cout<<"reference_progress_abs_: "<<reference_progress_abs_<<std::endl;
    // publish the progress to python
    std_msgs::Int32 progress_msg;
    progress_msg.data = reference_progress_abs_;
    ref_progress_pub_.publish(progress_msg);
    // for(std::list<quadrotor_common::TrajectoryPoint>::iterator iter = reference_trajectory.points.begin(); iter != reference_trajectory.points.end(); ++iter)
    //   std::cout<<iter->position.transpose()<<std::endl;
    // std::cout<<state_estimate.position.transpose()<<std::endl;
  ...
  if (state_machine_ == StateMachine::kExecuteExpert ||
    ...
    // std::cout<<"reference_progress_abs_: "<<reference_progress_abs_<<"    "<<(acrobatic_trajectory_.points.size())<<std::endl;
    if ((acrobatic_trajectory_.points.back().position - state_estimate.position).norm() < 0.8 ||
        (reference_progress_abs_ > (acrobatic_trajectory_.points.size()-6))) {
    ...
    // std::cout<<"reference_trajectory.points.front(): "<<reference_trajectory.points.front().position.transpose()<<std::endl;
    ...
    // std::cout<<"control_cmd.expected_execution_time: "<<control_cmd.expected_execution_time<<std::endl;
...
void AgileAutonomyAB::completedGlobalPlanCallback(
...
quadrotor_common::QuadStateEstimate AgileAutonomyAB::getPredictedStateEstimate(
    const ros::Time& time, const state_predictor::StatePredictor* predictor) {
  return predictor->predictState(time);
}

void AgileAutonomyAB::publishControlCommand(
...
void AgileAutonomyAB::trajectoryCallback(
...
bool AgileAutonomyAB::selectBestNetworkPrediction(
...
bool AgileAutonomyAB::convertTrajectoriesToWorldFrame(
    const std::vector<quadrotor_common::Trajectory>& nw_trajectories,
...
bool AgileAutonomyAB::loadParameters() {
...
{
  ros::init(argc, argv, "agile_autonomy_ab");
  agile_autonomy_ab::AgileAutonomyAB agile_autonomy_ab;
